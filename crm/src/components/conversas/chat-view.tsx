    ...
    <div className="flex-1 flex flex-col h-full bg-gray-950">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-gray-900 border-b border-gray-800">
        ...
        <div className="flex-1">
          <h2 className="text-white font-medium text-sm">{displayName}</h2>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        {loading && (
          <p className="text-gray-500 text-sm text-center py-8">Carregando mensagens...</p>
        )}
        {!loading && messages.length === 0 && (
          <p className="text-gray-500 text-sm text-center py-8">Nenhuma mensagem.</p>
        )}
        {messages.map((msg) => (
          <div
            key={msg.key.id}
            ...
            >
              ...
              <p
                className={`text-xs mt-1 ${
                  msg.key.fromMe ? "text-violet-200" : "text-gray-500"
                }`}
              >
      ...
      <div className="px-4 py-3 bg-gray-900 border-t border-gray-800">
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 bg-gray-800 text-white text-sm rounded-2xl px-4 py-2.5 placeholder-gray-500 outline-none focus:ring-1 focus:ring-violet-500 resize-none max-h-32"
          />
          <button
            onClick={handleSend}
            disabled={sending || !text.trim()}
            className="bg-violet-600 text-white p-2.5 rounded-full hover:bg-violet-700 disabled:opacity-50 flex-shrink-0"
          >
