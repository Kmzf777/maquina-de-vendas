...
**Data:** 2026-06-17
**Branch:** `feat/melhorias-registro-vendas`
...
A execução é via **subagents**. Todo agente que mexer no frontend **deve usar a skill `frontend-design` e os primitivos shadcn**. Os pontos visuais (botão verde no deal, botão de registro no chat/painel, modal unificado) são desenhados pelos agentes frontend-design seguindo a paleta quente.
