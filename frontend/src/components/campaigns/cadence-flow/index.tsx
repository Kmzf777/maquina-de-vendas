...
function FlowBuilderInner({ campaignId }: { campaignId: string }) {
  ...
  useEffect(() => {
    setFlowHandlers({ addNodeBelow });
    return () => { setFlowHandlers({ addNodeBelow: null }); };
  }, [addNodeBelow]);
  ...
  useEffect(() => {
    setFlowHandlers({ deleteEdge });
    return () => { setFlowHandlers({ deleteEdge: null }); };
  }, [deleteEdge]);

  // Manter selectNode atualizado para CampaignFlowNode acessar sem closure stale
  useEffect(() => {
    setFlowHandlers({ selectNode: (nodeId: string) => setSelectedNodeId(nodeId) });
    return () => { setFlowHandlers({ selectNode: null }); };
  }, []);
