...
export function describeLeadOrigin(lead: LeadOriginInput, campaignName: string | null): LeadOrigin {
  ...
  // Mesma ordem de derive_channel: gclid > click-ids/anúncio Meta > utm de anúncio.
  const google = { kind: "pago", channel: "Google Ads", detail: paidDetail, page } as const;
  const meta = { kind: "pago", channel: "Meta Ads", detail: paidDetail, page } as const;
